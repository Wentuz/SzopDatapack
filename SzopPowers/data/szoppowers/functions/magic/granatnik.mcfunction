...
execute if entity @p[scores={spellslot=1..}] run effect give @p[scores={spellslot=1..}] hunger 5 100
...
summon tnt ^ ^1 ^1 {Tags:["projectile"],Fuse:30,Passengers:[{id:"tnt_minecart",Passengers:[{id:"tnt_minecart"}]}]}

# apply motion to projectile
execute store result entity @e[type=tnt,tag=projectile,limit=1] Motion[0] double 0.001 run scoreboard players get #targetX pos
execute store result entity @e[type=tnt,tag=projectile,limit=1] Motion[1] double 0.001 run scoreboard players get #targetY pos
execute store result entity @e[type=tnt,tag=projectile,limit=1] Motion[2] double 0.001 run scoreboard players get #targetZ pos
